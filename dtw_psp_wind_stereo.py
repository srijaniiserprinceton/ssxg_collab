...
def dtw_Bvectors(Bdict_sat1, Bdict_sat2, downsampling_factor=1000):
    '''
    ...
    for i in range(3):
        B1 = Bdict_sat1['B'][i,::downsampling_factor]
        B2 = Bdict_sat2['B'][i,::downsampling_factor]
...
if __name__=='__main__':
    ...
    # loading PSP data
    # PSP_data_dict = load_data.get_PSP_data(tstart_psp, tend_psp, data_products)
    ...
    plot_FN.plot_Bfields_allsat(psp=StereoA_data_dict, wind=StereoA_data_dict, stereoA=StereoA_data_dict)

    # calculating the alignments using dtw
    alignment_dict_stereo_stereo = dtw_Bvectors(StereoA_data_dict, StereoA_data_dict, downsampling_factor=1000)

    for key in alignment_dict_stereo_stereo.keys():
        alignment_dict_stereo_stereo[key].plot(type="threeway")
